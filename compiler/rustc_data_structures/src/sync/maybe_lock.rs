...
/// A guard holding mutable access to a `MaybeLock` which is in a locked state.
#[must_use = "if unused the MaybeLock will immediately unlock"]
pub struct MaybeLockGuard<'a, T> {
    lock: &'a MaybeLock<T>,
...
impl<'a, T: 'a> Deref for MaybeLockGuard<'a, T> {
    type Target = T;
    #[inline]
    fn deref(&self) -> &T {
...
impl<'a, T: 'a> DerefMut for MaybeLockGuard<'a, T> {
    #[inline]
...
impl<'a, T: 'a> Drop for MaybeLockGuard<'a, T> {
    #[inline]
    fn drop(&mut self) {
        ...
        match self.mode {
            Mode::NoSync => {
                let cell = unsafe { &self.lock.mode_union.no_sync.0 };
...
union ModeUnion {
    /// Indicates if the cell is locked. Only used if `MaybeLock.mode` is `NoSync`.
    no_sync: ManuallyDrop<(Cell<bool>, Option<ThreadId>)>,

    /// A lock implementation that's only used if `MaybeLock.mode` is `Sync`.
...
fn setup_id() -> Option<ThreadId> {
    THREAD_ID.with(|id| {
        if unlikely(id.get().is_none()) {
            outline(|| {
                THREAD_ID.with(|id| {
                    id.set(Some(thread::current().id()));
                });
            })
        }
        id.get()
...
impl<T> MaybeLock<T> {
    ...
    pub fn new(inner: T, enabled: bool) -> Self {
        let (mode, mode_union) = if enabled && mode::might_be_dyn_thread_safe() {
            ...
            (
                Mode::NoSync,
                ModeUnion { no_sync: ManuallyDrop::new((Cell::new(!LOCKED), setup_id())) },
            )
        };
        MaybeLock { mode, mode_union, data: UnsafeCell::new(inner) }
    }
    ...
    unsafe fn check(&self) {
        ...
        #[cold]
        fn multiple_threads() -> ! {
            panic!("maybe lock used on multiple thread while being disabled")
        }

        let id = unsafe { self.mode_union.no_sync.1 };
        if unlikely(THREAD_ID.with(|id| id.get()) != id) {
            multiple_threads();
        }
    }

    #[inline(always)]
    pub fn try_lock(&self) -> Option<MaybeLockGuard<'_, T>> {
        let mode = self.mode;
        ...
        match mode {
            Mode::NoSync => {
                ...
                let cell = unsafe { &self.mode_union.no_sync.0 };
        ...
        .then(|| MaybeLockGuard { lock: self, marker: PhantomData, mode })
    }
    ...
    #[inline(always)]
    #[track_caller]
    pub unsafe fn lock_assume(&self, mode: Mode) -> MaybeLockGuard<'_, T> {
        #[inline(never)]
        #[track_caller]
        #[cold]
        fn lock_held() -> ! {
        ...
        unsafe {
            match mode {
                Mode::NoSync => {
                    if unlikely(self.mode_union.no_sync.0.replace(LOCKED) == LOCKED) {
                        lock_held()
                    }
                    self.check();
                }
        ...
        MaybeLockGuard { lock: self, marker: PhantomData, mode }
    }

    #[inline(always)]
    #[track_caller]
    pub fn lock(&self) -> MaybeLockGuard<'_, T> {
        unsafe { self.lock_assume(self.mode) }
    }
}

unsafe impl<T: DynSend> DynSend for MaybeLock<T> {}
unsafe impl<T: DynSend> DynSync for MaybeLock<T> {}

impl<T: fmt::Debug> fmt::Debug for MaybeLock<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.try_lock() {
            Some(guard) => f.debug_struct("MaybeLock").field("data", &&*guard).finish(),
            None => {
                ...
                f.debug_struct("MaybeLock").field("data", &LockedPlaceholder).finish()
            }
        }
    }
}
