                                  ...
                                  -> Vec<Constructor<'tcx>>
{
    ...
    match pcx.ty.sty {
        ...
        ty::TyArray(ref sub_ty, len) if len.to_usize(cx.tcx).is_some() => {
...
fn slice_pat_covered_by_constructor<'tcx>(
    ...
    {
        match pat.kind {
            box PatternKind::Constant { value } => if let Some(b) = value.to_bits(pat.ty) {
                assert_eq!(b as u8 as u128, b);
                if b as u8 != *ch {
                    return Ok(false);
                }
            } else {
                span_bug!(pat.span, "bad const u8 {:?}", value)
            },
            _ => {}
        }
    }
