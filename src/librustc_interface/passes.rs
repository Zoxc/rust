...
use rustc::hir::def_id::{CrateNum, LocalCrate, LOCAL_CRATE};
use rustc::lint;
...
use rustc::session::config::{self, Input, OutputFilenames, OutputType, InputsAndOutputs};
...
fn expand_macros<'tcx>(
    tcx: TyCtxt<'_, 'tcx, 'tcx>,
    _: LocalCrate,
) -> Result<Lrc<ty::ExpansionResult>> {
    let (krate, plugin_info) = tcx.register_plugins(LocalCrate)?.steal();
    let sess = tcx.sess_rc.clone();
    let cstore_rc: &'tcx dyn Any = tcx.cstore_rc;
    ...
    let crate_name = tcx.early_crate_name(LocalCrate)?.to_string();
...
fn register_plugins<'tcx>(
    tcx: TyCtxt<'_, 'tcx, 'tcx>,
    _: LocalCrate,
) -> Result<Lrc<Steal<(ast::Crate, ty::PluginInfo)>>> {
    let crate_name = tcx.early_crate_name(LocalCrate)?.as_str();
    let mut krate = tcx.parse(LocalCrate)?.steal();
    let cstore_rc: &'tcx dyn Any = tcx.cstore_rc;
    ...
    tcx.dep_graph_future(LocalCrate);
...
fn lower_ast_to_hir<'tcx>(
    ...
    Ok(tcx.global_arenas.lowered_hir.alloc(hir::LoweredHir {
        forest,
        export_map: resolutions.export_map.into_iter().map(|(k, v)| {
            (k, Lrc::new(v))
        }).collect(),
        freevars: resolutions.freevars.into_iter().map(|(k, v)| {
            (defs.local_def_id(k), Lrc::new(v))
        }).collect(),
...
fn prepare_outputs<'tcx>(
    ...
    let outputs = util::build_output_filenames(
        ...
        &tcx.io.output_dir,
        &tcx.io.output_file,
        &tcx.expand_macros(LocalCrate)?.ast_crate.borrow().attrs,
        tcx.sess
    );
