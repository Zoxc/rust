...
use rustc::middle::const_val::{ConstVal, ConstValue, ConstEvalErr};
use rustc_mir::interpret::{read_target_uint, const_val_field};
use rustc::hir::def_id::DefId;
use rustc::mir;
use rustc_data_structures::indexed_vec::Idx;
use rustc::mir::interpret::{GlobalId, MemoryPointer, PrimVal, Allocation};
...
fn const_value_to_llvm<'tcx>(cx: &CodegenCx<'_, 'tcx>, val: ConstValue, ty: Ty<'tcx>) -> ValueRef {
    ...
    match val {
        ...
        ConstValue::ByRef(alloc) => global_initializer(cx, alloc),
    }
}
        ...
        self.mir_constant_to_const_value(bx, constant)
            .and_then(|c| {
                ...
                let values: Result<Vec<ValueRef>, _> = (0..fields).map(|field| {
                    ...
                    if let Some(prim) = field.to_primval(bx.tcx()) {
