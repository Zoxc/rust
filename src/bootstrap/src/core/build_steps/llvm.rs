...
use crate::{CLang, Crt, CrtMode, GitRepo, Kind, exit, trace};
...
impl CommandLineStep for Llvm {
    ...
    fn run(self, builder: &Builder<'_>) -> LlvmResult {
        ...
        // FIXME: This check assumes static is the default when unspecified
        if target.contains("musl") && builder.crt_static(target, CrtMode::Compiler) != Crt::Dynamic
        {
...
fn configure_cmake(
    ...
    if target.is_msvc() {
        let static_crt = builder.crt_static(target, crt_mode) == Crt::Static;
        cfg.define(
    ...
    for flag in builder
        .cc_handled_cflags(target, CLang::C, crt_mode)
        .into_iter()
    ...
    for flag in builder
        .cc_handled_cflags(target, CLang::Cxx, crt_mode)
        .into_iter()
...
impl CommandLineStep for Enzyme {
    ...
    fn run(self, builder: &Builder<'_>) -> Self::Output {
        ...
        configure_cmake(builder, target, CrtMode::Compiler, &mut cfg, true, ldflags, cflags, &[]);
