    ...
    -> Progress<'tcx>
{
    ...
    let trait_ref = ty::TraitRef {
        def_id: gen_def_id,
        substs: tcx.mk_substs_trait(obligation.predicate.trait_ref.self_ty(), &[]),
    };

    let gen_sig = gen_sig.skip_binder(); // FIXME

    let ty = if obligation.predicate.item_name == Symbol::intern("Return") {
        gen_sig.return_ty
    } else if obligation.predicate.item_name == Symbol::intern("Yield") {
        gen_sig.suspend_ty
    } else {
        panic!()
    };
    ...
    let predicate = ty::Binder(ty::ProjectionPredicate {
        projection_ty: ty::ProjectionTy {
            trait_ref: trait_ref,
            item_name: obligation.predicate.item_name,
        },
        ty: ty
    });
