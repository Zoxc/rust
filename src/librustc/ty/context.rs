...
use crate::middle;
use crate::hir::{TraitCandidate, HirId, ItemKind, ItemLocalId, Node, LoweredHir};
use crate::hir::def::Def;
use crate::hir::def_id::{CrateNum, LocalCrate, DefId, LOCAL_CRATE};
...
pub struct GlobalArenas<'tcx> {
    pub lowered_hir: TypedArena<LoweredHir>,
...
impl<'a, 'gcx, 'tcx> TyCtxt<'a, 'gcx, 'tcx> {
    ...
    pub fn def_path_hash_to_def_id(self) -> Option<&'gcx FxHashMap<DefPathHash, DefId>> {
...
pub fn provide(providers: &mut ty::query::Providers<'_>) {
    providers.in_scope_traits_map = |tcx, id| {
        tcx.lowered_hir().trait_map.get(&id).cloned()
    };
    providers.module_exports = |tcx, id| {
        tcx.lowered_hir().export_map.get(&id).cloned()
    };
    ...
    providers.freevars = |tcx, id| {
        tcx.lowered_hir().freevars.get(&id).cloned()
    };
    ...
    providers.maybe_unused_extern_crates = |tcx, cnum| {
        ...
        Lrc::new(tcx.lowered_hir().maybe_unused_extern_crates.clone())
    };
    ...
    providers.output_filenames = |tcx, cnum| {
        assert_eq!(cnum, LOCAL_CRATE);
        tcx.prepare_outputs(LocalCrate).unwrap()
    };
