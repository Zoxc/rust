...
impl<'a, 'tcx> Lift<'tcx> for ty::GenSig<'a> {
    ...
    fn lift_to_tcx<'b, 'gcx>(&self, tcx: TyCtxt<'b, 'gcx, 'tcx>) -> Option<Self::Lifted> {
        tcx.lift(&(self.suspend_ty, self.return_ty)).map(|(suspend_ty, return_ty)| {
            ty::GenSig {
                suspend_ty: suspend_ty,
                return_ty: return_ty,
            }
        })
    }
...
impl<'tcx> TypeFoldable<'tcx> for Ty<'tcx> {
    fn super_fold_with<'gcx: 'tcx, F: TypeFolder<'gcx, 'tcx>>(&self, folder: &mut F) -> Self {
        let sty = match self.sty {
            ...
            ty::TyGenerator(did, substs) => ty::TyGenerator(did, substs.fold_with(folder)),
    ...
    fn super_visit_with<V: TypeVisitor<'tcx>>(&self, visitor: &mut V) -> bool {
        match self.sty {
            ...
            ty::TyGenerator(_did, ref substs) => substs.visit_with(visitor),
