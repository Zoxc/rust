...
impl<'a, 'gcx, 'acx, 'tcx> ClosureSubsts<'tcx> {
    ...
    {
        let base_generics = tcx.generics_of(tcx.closure_base_def_id(def_id));
        self.substs[base_generics.count()..].iter().map(
...
/// In MIR, this may be None, since the value is no longer useful.
#[derive(Copy, Clone, PartialEq, Eq, Hash, Debug, RustcEncodable, RustcDecodable)]
