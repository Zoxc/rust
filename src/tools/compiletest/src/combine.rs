...
/*
pub fn run_combined_instance_timed(config: &Config,
                    instance: usize,
    ...
    println!("In worker {} with {}", instance, tests.len());
    ...
    for (i, test) in tests.into_iter().enumerate() {
        ...
        println!("test {:?} - took {} seconds to compile", paths.file, time.as_secs());

        times.push((paths.file, time));

        events.push(TestEvent::TeResult(test.0.desc, TestResult::TrOk, Vec::new()));
    }

    times.sort_by_key(|t| t.1);
    times.reverse();
    for (test, time) in times.into_iter().take(5) {
        println!("summary | test {:?} - took {} seconds to compile", test, time.as_secs());
    }
    (events, Vec::new(), 0)
}
*/
